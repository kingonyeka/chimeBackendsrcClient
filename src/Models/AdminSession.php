...
class AdminSession extends Database
{
    ...
    {
        try {
            $stmt = $this->db->prepare("SELECT * FROM {$this->table} WHERE access_token = :access_token");
    ...
    //     $stmt = $this->db->prepare("SELECT * FROM  {$this->table} WHERE user_id = ? AND refresh_token = ?");
    //     $stmt->execute([$userId, $token]);
    //     return $stmt->fetch(PDO::FETCH_ASSOC);
    // }

    public function getToken($userId, $accessToken, $refreshToken) {
        $stmt = $this->db->prepare("SELECT * FROM  {$this->table} WHERE user_id = ? AND access_token = ? AND refresh_token = ?");
        $stmt->execute([$userId, $accessToken, $refreshToken]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function getRefreshToken($userId, $token) {
        $stmt = $this->db->prepare("SELECT * FROM  {$this->table} WHERE user_id = ? AND refresh_token = ?");
        $stmt->execute([$userId, $token]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function getAccessToken($userId, $token) {
        $stmt = $this->db->prepare("SELECT * FROM  {$this->table} WHERE user_id = ? AND access_token = ?");
        $stmt->execute([$userId, $token]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
